...
void InstrimentationLogger::WriteToString(std::string & ret) {
	...
	ss << std::setfill(' ') << std::setw(30) << "Library Name" << " | " << std::setfill(' ') << std::setw(25) << "Function Name"
	   ...
	   << " | " << std::setfill(' ') << std::setw(10) << "LS Inst" 
	   << " | " << std::setfill(' ') << std::setw(10) << "Wrapped" << std::endl;
	for (auto i : _storage) {
		...
		ss << std::setfill(' ') << std::setw(30) << p.filename().string() << " | " << std::setfill(' ') << std::setw(25) << tmp << " | " << std::setfill(' ') << std::setw(10) << i.second[0] << " | " <<  std::setfill(' ') << std::setw(10) <<  i.second[1]  << " | " << std::setfill(' ') << std::setw(10) << i.second[2] 
		   << " | " << std::setfill(' ') << std::setw(10) << i.second[3] << std::endl;
	}
	ret = ss.str();
} 
