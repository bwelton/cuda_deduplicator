...
#include <cuda.h>
extern "C" {
int  DIOGENES_cudaFreeWrapper(void * mem) {
	std::cerr << "I am freeing an address of .... " << std::hex << (uint64_t) mem << std::endl;
	return cudaFree(mem);
}

int DIOGENES_cudaMallocWrapper(void ** mem, size_t size) {
...
int DIOGENES_cudaMemcpyAsyncWrapper(void * dst, const void * src, size_t size, cudaMemcpyKind kind, cudaStream_t stream) {
