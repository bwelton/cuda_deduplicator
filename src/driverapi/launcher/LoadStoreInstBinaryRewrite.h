...
class LoadStoreInstBinaryRewrite {
public: 
	...
	void InsertLoadStoresInit(std::vector<uint64_t> & skips, uint64_t & instUntil, std::vector<StackPoint> & points, std::map<uint64_t, StackRecord> & syncStacks, std::vector<std::string> wrappedFunctionNames);
private:
	void Setup();
